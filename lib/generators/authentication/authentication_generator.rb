class AuthenticationGenerator < Rails::Generators::Base
  ...
  def add_oauth_config_to_application_yml
    ...
    oauth_config = <<~YAML
      ...
      # Leave client id and secret blank to use Ultimate Social Media Auth fallback

      # Google OAuth
      GOOGLE_OAUTH_ENABLED: '<%= ENV.fetch("ULTIMATE_GOOGLE_OAUTH_ENABLED", "false") %>'
      GOOGLE_CLIENT_ID: '<%= ENV.fetch("ULTIMATE_AUTH_CLIENT_ID", "") %>'
      GOOGLE_CLIENT_SECRET: '<%= ENV.fetch("ULTIMATE_AUTH_CLIENT_SECRET", "") %>'

      # Facebook OAuth
      FACEBOOK_OAUTH_ENABLED: '<%= ENV.fetch("ULTIMATE_FACEBOOK_OAUTH_ENABLED", "false") %>'
      FACEBOOK_APP_ID: '<%= ENV.fetch("ULTIMATE_AUTH_CLIENT_ID", "") %>'
      FACEBOOK_APP_SECRET: '<%= ENV.fetch("ULTIMATE_AUTH_CLIENT_SECRET", "") %>'

      # Twitter OAuth
      TWITTER_OAUTH_ENABLED: '<%= ENV.fetch("ULTIMATE_TWITTER_OAUTH_ENABLED", "false") %>'
      TWITTER_API_KEY: '<%= ENV.fetch("ULTIMATE_AUTH_CLIENT_ID", "") %>'
      TWITTER_API_SECRET: '<%= ENV.fetch("ULTIMATE_AUTH_CLIENT_SECRET", "") %>'

      # GitHub OAuth
      GITHUB_OAUTH_ENABLED: '<%= ENV.fetch("ULTIMATE_GITHUB_OAUTH_ENABLED", "false") %>'
      GITHUB_CLIENT_ID: '<%= ENV.fetch("ULTIMATE_AUTH_CLIENT_ID", "") %>'
      GITHUB_CLIENT_SECRET: '<%= ENV.fetch("ULTIMATE_AUTH_CLIENT_SECRET", "") %>'
