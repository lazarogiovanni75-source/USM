...
# Poyo.ai Image Generation Service
# Uses GPT Image 1.5 via Poyo.ai unified API
# API Documentation: https://api.poyo.ai
class PoyoImageService
  ...
  # Generate image using GPT Image 1.5 via Poyo.ai unified API
  #
  ...
  def generate_gpt_image(prompt:,
    ...
    if task_id.present?
      Rails.logger.info "[PoyoImageService] Image generation started, task_id: #{task_id}"
      { 'task_id' => task_id, 'output' => nil, 'status' => 'not_started' }
    else
      Rails.logger.error "[PoyoImageService] No task_id in response: #{result.inspect}"
  ...
  def image_status(task_id)
    ...
    Rails.logger.info "[PoyoImageService] Task status response: #{result.inspect}"
    ...
    Rails.logger.error "[PoyoImageService] Status check error: #{e.message}"
  ...
  def handle_response(response)
    ...
    Rails.logger.debug "[PoyoImageService] Raw response code: #{response.code}, body: #{response.body}"
    ...
    when 200..299
      if parsed.is_a?(Hash) && (parsed['error'] || parsed['message'] || parsed['status'] == 'error')
        ...
        Rails.logger.error "[PoyoImageService] API returned error: #{error_msg}"
